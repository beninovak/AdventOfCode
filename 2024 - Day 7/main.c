...
int is_doable(_tokens* toks, uint64_t target, uint32_t last_index) {
    uint32_t last = atoi(toks->tokens[last_index]);
    printf("Checking target: %lu, last: %u", target, last);
    if (target % last != 0) {
        if (last_index > 1) {
        ...
        printf(" -> Divisible and returning cuz last!\n");
        return 0;
    }
}
